 ...
 *
 * ...AND THEN SOME, STILL DEVELOPING.
 */
...
int main(int argc, char *argv[]){
	string usage = " [phenotype output file] [# of generations] [sample size] [mutation rate] [fold threshold] [bind threshold] [protein bead file] [protein coding sequence] [ligand bead file] [decoy ligand bead file] [novel ligand bead file] [opt: sequence file to restore simulation from]";
	...
	ifstream novelLigBeadFile(argv[11]);

	/* Declare some variables.
	 */
	unsigned int i,j;
	int numDead, numNormal, numDecoy, numNeoP, numDecoyNeoP;
	...
	vector< shared_ptr<Structure> > novelLigandStructures;
	...
	*/
	...
	shared_ptr<TwoBeadStructure> novelLigandStructure(new TwoBeadStructure(TwoBeadStructure::parseBeadFile(novelLigBeadFile)));
	proteinBeadFile.close();
	ligandBeadFile.close();
	decoyLigBeadFile.close();
	novelLigBeadFile.close();
	...
	*/
	...
	novelLigandStructure->calcExposure(false);
	...
	wholeComplex.clear();
	wholeComplex.push_back(proteinStructure);
	wholeComplex.push_back(novelLigandStructure);
	wholeComplex = TwoBeadStructure::calcComplexExposure(wholeComplex);
	novelLigandStructure = wholeComplex[1];
	...
	if(argc == 12){
		...
		*/
		...
		novelLigandStructures.push_back(novelLigandStructure);
		...
		pOriginalSample = shared_ptr<MultipleStructureOrganismNeo>(new MultipleStructureOrganismNeo( mutRate, startingDna, startingStructures, ligandStructures, decoyLigandStructures, novelLigandStructures, engModel, foldThreshold, bindThreshold, geometry ));
	 ...
	 */
		 ...
		 * experiment.
		 */
		ifstream seqCollectionFile(argv[12]);
		...
		*/
		 ...
		 */
		seqCollectionFile.open(argv[12]);
		while( getline(seqCollectionFile, line) ){
			/* Don't read comment lines or empty lines.
			 */
			if(line.substr(0,1).compare("#") != 0 && line.length() > 1){
				...
				if(currentGeneration == lastGeneration){
					...
					*/
					...
					allOrganisms.push_back( shared_ptr<MultipleStructureOrganismNeo>(new MultipleStructureOrganismNeo(mutRate, startingDna, startingStructures, ligandStructures, decoyLigandStructures, novelLigandStructures, engModel, foldThreshold, bindThreshold, geometry)) );
				}
			}
		}
	...
	/* Prep the outfile.
	 */
	outFile << "Gen\tDead\tNormal\tDecoy\tNeoP\tDecoyNeoP" << endl;
	...
	//cout << "Gen\tID\tParentID\tPhenotype\tDNA\tProtein" << endl;
	...
	*/
	for(i = 0; i < pSimulation->getNumGenerations(); i++){
		...
		for(j = 0; j < currentPhenotypes.size(); j++){
			...
			else if (currentPhenotypes[j].compare("Original") == 0){
				numNormal++;
			}
			else if (currentPhenotypes[j].compare("Decoy") == 0){
				numDecoy++;
			}
			else if (currentPhenotypes[j].compare("Original+Neo1") == 0){
				numNeoP++;
			}
			else if (currentPhenotypes[j].compare("Decoy+Neo1") == 0){
				numDecoyNeoP++;
			}
		}
		 ...
		 */
		outFile << i+1 << "\t" << numDead << "\t" << numNormal << "\t" << numDecoy << "\t" << numNeoP << "\t" << numDecoyNeoP << endl;
		 ...
		 */
		//pSimulation->savePopulation(cout);
