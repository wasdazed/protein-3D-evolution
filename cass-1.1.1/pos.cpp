...
int main(int argc, char *argv[]){
	...
	vector<vector<float>> engTerms;
	vector<vector<float>> nativeEngTerms;
	vector<float> useTerms(7,0.0);
	 ...
	 */
	if(energyFunction.compare("folding") == 0){
		nativeEngTerms = pEngModel->getFoldScoreTerms_pos(nativeStructure);
...
//	cout << "#Native energy term values:" << endl;
	for(i = 0; i < nativeEngTerms.size(); i++){
		float s = 0;
//		cout << "#pos number:" << nativeEngTerms[i].size()<<endl;
		for(j = 0; j < nativeEngTerms[i].size()-1; j++){
			cout << nativeEngTerms[i][j] << "\t";
			s+=nativeEngTerms[i][j];
		}
		cout << nativeEngTerms[i][j] << endl;
		s+=nativeEngTerms[i][j];
//		cout << "# sum:" << s <<endl;
	}
...
///		engTerms = pEngModel->sampleFoldGapTerms_pos(sequence, bondAngles, resBBDists, resDists, threeMers, helixFourMers, backboneDists, betaFourMers, backboneTorsions, exposures, mersenneRandGen);
...
//		}
		...
		/* Print results to STDOUT.
		 */
///		for(i = 0; i < engTerms.size(); i++){
///				float s = 0;
//				cout << "#pos number:" << engTerms[i].size()<<endl;
///				for(j = 0; j < engTerms[i].size()-1; j++){
///					cout << engTerms[i][j] << "\t";
///					s+=engTerms[i][j];
///				}
///				cout << engTerms[i][j] << endl;
///				s+=engTerms[i][j];
//				cout << "# sum:" << s <<endl;
///			}
///	}
